...
function saveProfile(profile, originalId) {
    if (originalId && originalId.trim().length > 0) {
      ...
      WHERE id = ?
    `);
        updateStmt.run(profile.id, profile.name, profile.age, profile.role, profile.verification ? 1 : 0, originalId.trim());
    }
    else {
    ...
    `);
        insertStmt.run(profile.id, profile.name, profile.age, profile.role, profile.verification ? 1 : 0);
    }
}
